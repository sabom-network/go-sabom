...
import (
	...
	"github.com/sabom-network/go-sabom/accounts/usbwallet/trezor"
...
// or whether the Sabom app was not started on it.
...
// Sabom address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.SabomAddress)
	if _, err := w.trezorExchange(&trezor.SabomGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.SabomTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.SabomTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
