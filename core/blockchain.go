...
// Package core implements the Sabom consensus protocol.
package core
...
import (
	...
	"github.com/sabom-network/go-sabom/common/prque"
	...
	"github.com/sabom-network/go-sabom/internal/syncx"
...
// available in the database. It initialises the default Sabom Validator
// and Processor.
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Gsab is initialized with
...
// it's not checked in the Gsab side anymore.
...
// sync, Gsab will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Gsab with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Gsab to a new version which supports transaction
