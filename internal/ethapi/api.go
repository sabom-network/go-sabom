...
import (
	...
	"github.com/sabom-network/go-sabom/accounts"
	"github.com/sabom-network/go-sabom/accounts/abi"
	"github.com/sabom-network/go-sabom/accounts/keystore"
	"github.com/sabom-network/go-sabom/accounts/scwallet"
	"github.com/sabom-network/go-sabom/common"
	"github.com/sabom-network/go-sabom/common/hexutil"
	"github.com/sabom-network/go-sabom/common/math"
	"github.com/sabom-network/go-sabom/consensus/ethash"
	"github.com/sabom-network/go-sabom/consensus/misc"
	"github.com/sabom-network/go-sabom/core"
	"github.com/sabom-network/go-sabom/core/state"
	"github.com/sabom-network/go-sabom/core/types"
	"github.com/sabom-network/go-sabom/core/vm"
	"github.com/sabom-network/go-sabom/crypto"
	"github.com/sabom-network/go-sabom/eth/tracers/logger"
	"github.com/sabom-network/go-sabom/log"
	"github.com/sabom-network/go-sabom/p2p"
	"github.com/sabom-network/go-sabom/params"
	"github.com/sabom-network/go-sabom/rlp"
	"github.com/sabom-network/go-sabom/rpc"
	"github.com/tyler-smith/go-bip39"
)

// SabomAPI provides an API to access Sabom related information.
type SabomAPI struct {
	b Backend
}

// NewSabomAPI creates a new Sabom protocol API.
func NewSabomAPI(b Backend) *SabomAPI {
	return &SabomAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *SabomAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *SabomAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *SabomAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *SabomAPI) Syncing() (interface{}, error) {
...
// SabomAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type SabomAccountAPI struct {
	am *accounts.Manager
}

// NewSabomAccountAPI creates a new SabomAccountAPI.
func NewSabomAccountAPI(am *accounts.Manager) *SabomAccountAPI {
	return &SabomAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *SabomAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Sabom ECDSA signature for:
// keccak256("\x19Sabom Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/sabom-network/go-sabom/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Sabom Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/sabom-network/go-sabom/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Sabom signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gsab.
...
// BlockChainAPI provides an API to access Sabom blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Sabom blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Sabom chain config.
//
...
// See: https://github.com/sabom-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/sabom-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Sabom APIs exposed over the debugging
// namespace.
...
// Version returns the current sabom protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Sab)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f sab) exceeds the configured cap (%.2f sab)", feeFloat, cap)
	}
	return nil
}
