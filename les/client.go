// Copyright 2019 The go-sabom Authors
...
// Package les implements the Light Sabom Subprotocol.
package les
...
import (
	...
	"github.com/sabom-network/go-sabom/common/mclock"
	"github.com/sabom-network/go-sabom/consensus"
	"github.com/sabom-network/go-sabom/core"
	"github.com/sabom-network/go-sabom/core/bloombits"
	...
	"github.com/sabom-network/go-sabom/internal/shutdowncheck"
	"github.com/sabom-network/go-sabom/les/downloader"
	"github.com/sabom-network/go-sabom/les/vflux"
	vfc "github.com/sabom-network/go-sabom/les/vflux/client"
	"github.com/sabom-network/go-sabom/light"
...
func New(stack *node.Node, config *ethconfig.Config) (*LightSabom, error) {
...
func (s *LightSabom) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightSabom) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightSabom) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the sabom package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightSabom) APIs() []rpc.API {
...
func (s *LightSabom) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightSabom) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightSabom) TxPool() *light.TxPool              { return s.txPool }
func (s *LightSabom) Engine() consensus.Engine           { return s.engine }
func (s *LightSabom) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightSabom) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightSabom) EventMux() *event.TypeMux           { return s.eventMux }
func (s *LightSabom) Merger() *consensus.Merger          { return s.merger }

// Protocols returns all the currently configured network protocols to start.
func (s *LightSabom) Protocols() []p2p.Protocol {
...
// light sabom protocol implementation.
func (s *LightSabom) Start() error {
