...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gsab is "gsab". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gsab", keep that.
	if name == "gsab" || name == "gsab-testnet" {
		name = "Gsab"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gsab" instances.
var isOldGsabResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gsab 1.4 are used if they exist.
	if warn, isOld := isOldGsabResource[path]; isOld {
		oldpath := ""
		if c.name() == "gsab" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGsabResourceWarning, "Using deprecated resource file %s, please move this file to the 'gsab' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-sabom-keystore")
		isEphemeral = true
	}
