...
import (
	...
	"github.com/sabom-network/go-sabom/accounts/external"
	"github.com/sabom-network/go-sabom/accounts/keystore"
	"github.com/sabom-network/go-sabom/accounts/scwallet"
	"github.com/sabom-network/go-sabom/accounts/usbwallet"
...
func loadConfig(file string, cfg *gsabConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gsab.ipc"
	return cfg
}

// makeConfigNode loads gsab configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gsabConfig) {
	// Load defaults.
	cfg := gsabConfig{
...
// makeFullNode loads gsab configuration and creates the Sabom backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gsab db freezer-migrate`.")
		}
	}
	...
	// Add the Sabom Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gsabConfig) {
