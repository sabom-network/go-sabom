...
import (
	...
	"github.com/sabom-network/go-sabom/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	gsab, err := runGsab()
	if err != nil {
		t.Fatalf("could not run gsab: %v", err)
	}
	defer gsab.Close()

	suite, err := NewSuite(gsab.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGsab creates and starts a gsab node
func runGsab() (*node.Node, error) {
...
func setupGsab(stack *node.Node) error {
