...
// Package utils contains internal helper functions for go-sabom commands.
package utils
...
import (
	...
	"github.com/sabom-network/go-sabom/common/fdlimit"
	"github.com/sabom-network/go-sabom/consensus"
	"github.com/sabom-network/go-sabom/consensus/ethash"
	"github.com/sabom-network/go-sabom/core"
	"github.com/sabom-network/go-sabom/core/rawdb"
	"github.com/sabom-network/go-sabom/core/vm"
	"github.com/sabom-network/go-sabom/crypto"
	"github.com/sabom-network/go-sabom/eth"
	ethcatalyst "github.com/sabom-network/go-sabom/eth/catalyst"
	"github.com/sabom-network/go-sabom/eth/downloader"
	"github.com/sabom-network/go-sabom/eth/ethconfig"
	"github.com/sabom-network/go-sabom/eth/filters"
	"github.com/sabom-network/go-sabom/eth/gasprice"
	"github.com/sabom-network/go-sabom/eth/tracers"
	"github.com/sabom-network/go-sabom/ethdb"
	"github.com/sabom-network/go-sabom/ethdb/remotedb"
	"github.com/sabom-network/go-sabom/ethstats"
	"github.com/sabom-network/go-sabom/graphql"
	"github.com/sabom-network/go-sabom/internal/ethapi"
	"github.com/sabom-network/go-sabom/internal/flags"
	"github.com/sabom-network/go-sabom/les"
	lescatalyst "github.com/sabom-network/go-sabom/les/catalyst"
	"github.com/sabom-network/go-sabom/log"
	"github.com/sabom-network/go-sabom/metrics"
	"github.com/sabom-network/go-sabom/metrics/exp"
	"github.com/sabom-network/go-sabom/metrics/influxdb"
	"github.com/sabom-network/go-sabom/miner"
	"github.com/sabom-network/go-sabom/node"
	"github.com/sabom-network/go-sabom/p2p"
	"github.com/sabom-network/go-sabom/p2p/enode"
	"github.com/sabom-network/go-sabom/p2p/nat"
	"github.com/sabom-network/go-sabom/p2p/netutil"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in sab) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gsab will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gsab configurations storing the
...
// for Gsab and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gsab account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Sabom client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Sabom) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Sabom service: %v", err)
		}
...
// RegisterEthStatsService configures the Sabom Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Sabom Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gsab.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gsab.", tagsMap)
		}
