...
// information related to the Sabom protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(sabom.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as sabom.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria sabom.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(sabom.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
