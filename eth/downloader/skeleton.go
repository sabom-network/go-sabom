...
// the process. This is usually the case when Gsab is shutting down and some events
...
//
// Opposed to the original Sabom block synchronization which is trustless (and
...
// not from within (vs. legacy Sabom sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gsab is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gsab requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
