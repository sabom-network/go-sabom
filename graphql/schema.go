...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Sabom address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Sabom account at a particular block.
    type Account {
    ...
    # Transaction is an Sabom transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Sabom account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Sabom account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Sabom block by number or by hash. If neither is
