...
import (
	...
	"github.com/sabom-network/go-sabom/core/beacon"
...
type ethNode struct {
	...
	ethBackend *eth.Sabom
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightSabom
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Sabom, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Sabom node
	...
	// Create the node and configure a full Sabom node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightSabom, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Sabom node
	datadir, _ := os.MkdirTemp("", "")
