...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gsab.LightScryptN, Gsab.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.sabom", new NodeConfig());
			node.start();
			...
			SabomClient ec = node.getSabomClient();
	...
	// signatures too. Regression test for go-sabom issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/sabom/gsabtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gsab and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.sabom", "github.com/sabom-network/go-sabom/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gsab.aar"), "gsab.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "sabom", "gsabtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
