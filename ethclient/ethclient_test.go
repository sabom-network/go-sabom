...
// Verify that Client implements the sabom interfaces.
var (
	_ = sabom.ChainReader(&Client{})
	_ = sabom.TransactionReader(&Client{})
	_ = sabom.ChainStateReader(&Client{})
	_ = sabom.ChainSyncReader(&Client{})
	_ = sabom.ContractCaller(&Client{})
	_ = sabom.GasEstimator(&Client{})
	_ = sabom.GasPricer(&Client{})
	_ = sabom.LogFilterer(&Client{})
	_ = sabom.PendingStateReader(&Client{})
	// _ = sabom.PendingStateEventer(&Client{})
	_ = sabom.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Sabom Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new sabom service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == sabom.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != sabom.NotFound {
		t.Fatal("error should be sabom.NotFound")
	}
}
