...
// Package gsabclient provides an RPC client for gsab-specific APIs.
package gsabclient
...
// Client is a wrapper around rpc.Client that implements gsab-specific functionality.
//
// If you want to use the standardized Sabom RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg sabom.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg sabom.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gsab node.
...
// MemStats retrieves the current memory stats from a gsab node.
...
// GetNodeInfo retrieves the node info of a gsab node.
